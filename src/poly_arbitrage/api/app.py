...
from poly_arbitrage.runtime.bootstrap import ApplicationContainer, build_container
...
def create_app(container: ApplicationContainer | None = None) -> FastAPI:
    ...
    async def list_sources() -> dict[str, list[str]]:
        return {
            "batch": _container(app).ingestion.list_batch_jobs(),
            "stream": _container(app).ingestion.list_stream_jobs(),
        }
    ...
    async def run_job(job_name: str) -> dict[str, object]:
        if job_name not in _container(app).ingestion.list_batch_jobs():
            raise HTTPException(status_code=404, detail="job not found")
        result = await _container(app).ingestion.run_job(job_name)
        return {
    ...
    async def list_runs() -> list[dict[str, object]]:
        return await _container(app).store.list_runs()

    @app.get("/runs/{run_id}")
    async def get_run(run_id: str) -> dict[str, object]:
        run = await _container(app).store.get_run(run_id)
        if run is None:
...
def _container(app: FastAPI) -> ApplicationContainer:
